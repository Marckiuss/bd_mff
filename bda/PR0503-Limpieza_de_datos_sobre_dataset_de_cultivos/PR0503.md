...
```
    ...
    26/01/29 09:06:49 WARN NativeCodeLoader: Unable to load native-hadoop library for your platform... using builtin-java classes where applicable
...
```
    ...
    26/01/29 09:06:52 WARN CSVHeaderChecker: CSV header does not conform to the schema.
...
Crea una nueva columna llamada Crop_ID en un nuevo DataFrame df_eng.
Este ID debe seguir este formato estricto: **CODIGO_REGION-CULTIVO**.
...
# Visualizamos el resultado para confirmar el formato XXC-MAIZE
...
```
    ...
    26/01/29 09:06:53 WARN CSVHeaderChecker: CSV header does not conform to the schema.
     Header: Crop, Region
...
```
    ...
    26/01/29 09:07:01 WARN GarbageCollectionMetrics: To enable non-built-in garbage collector(s) List(G1 Concurrent GC), users should configure it(them) to spark.eventLog.gcMetrics.youngGenerationGarbageCollectors or spark.eventLog.gcMetrics.oldGenerationGarbageCollectors
